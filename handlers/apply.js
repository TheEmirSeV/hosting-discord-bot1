...
module.exports = client => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    if(!index) {
      if(!interaction.replied) return interaction?.reply({ephemeral: true, content: `❎ Could not find the Database for your Application!`});
      else return interaction?.editReply({ephemeral: true, content: `❎ Could not find the Database for your Application!`});
    }
